...
let aTotal = aP * Math.pow(1 + aR, n); 
for (let i = 1; i <= n; i++) { 
aTotal += aM * Math.pow(1 + aR, n - i); 
} \


let bTotal = bP * Math.pow(1 + bR, n); 
for (let i = 1; i <= n; i++) { 
bTotal += bM * Math.pow(1 + bR, n - i); 
} \
...
document.getElementById('comparisonResult').innerHTML = \ <p>Investment A after ${y} years: $${aTotal.toFixed(2)}</p> \ <p>Investment B after ${y} years: $${bTotal.toFixed(2)}</p> \ <p><strong>Better return: ${winner}</strong></p> \; 
});
